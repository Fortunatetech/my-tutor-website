...
export default function Header() {
  ...
  const links = [{href:'/',label:'Home'},{href:'/about',label:'About'},{href:'/services',label:'Services'},{href:'/blog',label:'Blog'},{href:'/contact',label:'Contact'}];
    ...
    <header className="sticky top-0 z-50 backdrop-blur-md bg-white/60 dark:bg-black/50">
      <div className="container mx-auto px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center space-x-6">
          {links.map(l=>(
            <Link key={l.href} href={l.href} className={`text-neutral-900 dark:text-neutral-100 hover:text-brand-500 ${pathname===l.href?'font-semibold':''}`}>{l.label}</Link>
          ))}
        </nav>
      ...
      {open && (
        <div className="md:hidden bg-white/95 dark:bg-black/90 border-t py-4 px-6">
          <div className="flex flex-col gap-3">
            {links.map(l => <Link key={l.href} href={l.href} className="text-neutral-900 dark:text-neutral-100">{l.label}</Link>)}
          </div>
        </div>
      )}
    </header>
