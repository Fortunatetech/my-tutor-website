...
import EmbeddedScheduler from "@/components/booking/EmbeddedSchedular";
    ...
    try {
      const res = await fetch('/api/booking', {
        ...
        body: JSON.stringify({
          name: prefillName,
          email: prefillEmail,
          service: serviceParam || SERVICES[0]?.id,
          message,
        }),
      });
  ...
  return (
    <section className="py-16">
      <div className="container mx-auto px-6 lg:px-8">
        <h1 className="text-3xl font-bold text-brand-900 mb-4">Book a time</h1>
        <p className="text-neutral-600 mb-6">
          Use the scheduler below to pick an available slot. Or send a quick message and I’ll reply with options.
        </p>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          <div>
            {serviceObj && (
              <div className="mb-4">
                ...
                <div className="font-semibold">{serviceObj.title}</div>
              </div>
            )}

            {sent ? (
              <div className="p-4 bg-green-50 rounded">Thanks — we received your request. I will follow up soon.</div>
            ) : (
              <form onSubmit={handleSend} className="space-y-4">
                <div>
                  <label className="text-sm font-medium">Name</label>
                  <input value={prefillName} onChange={(e) => setPrefillName(e.target.value)} className="w-full mt-1 p-3 border rounded-lg" required />
                </div>

                <div>
                  <label className="text-sm font-medium">Email</label>
                  <input type="email" value={prefillEmail} onChange={(e) => setPrefillEmail(e.target.value)} className="w-full mt-1 p-3 border rounded-lg" required />
                </div>

                <div>
                  <label className="text-sm font-medium">Message / preferred times</label>
          ...
          <div>
            <div className="mb-4">
              <h2 className="text-lg font-semibold">Or schedule now</h2>
              <p className="text-sm text-neutral-600">Pick a time on my available calendar (opens inside this page).</p>
            </div>

            {/* NEW: Embedded scheduler component */}
            <EmbeddedScheduler serviceId={serviceParam} />
          </div>
        </div>
      </div>
    </section>
